...
const components = [...getAllFiles(docsDir), ...getAllFiles(uiDir)]
  ...
  .reduce(
    (acc, filePath) => {
      ...
      // Modify key to exclude the initial 'docs/' or 'ui/'
      ...
      // Add to the TypeScript components list only if it's a doc
      if (type === 'docs') {
      ...
      // Add to the JSON components list with type
...
// Generate TypeScript file with lazy-loaded components for docs only
...
// Generate JSON file with component data from both docs and ui, including the type field
