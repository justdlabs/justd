...
const ColorItem = ({ showItem, swatchClassName, item, name, isForTailwindVariable, selectedFormat }: ColorItemProps) => {
  const [copied, setCopied] = React.useState(false)
  const handleCopy = async (selectedColor: string) => {
    const toCopy = isForTailwindVariable ? formatColorForTailwind(selectedColor, selectedFormat as FormatOnlyForTailwindVariableType) : selectedColor
  ...
  return (
    <ListBoxItem
      textValue={name}
      className={cn('group w-full focus:outline-none cursor-pointer focus-visible:outline-primary-600 focus-visible:outline-2 focus-visible:outline-offset-1 rounded-md relative')}
      onAction={() => handleCopy(parseColor(item.color as string)?.toString(selectedFormat))}
      style={{
        color: textColorBasedOnBg(item.color)
      }}
    >
      <ColorSwatch aria-label={`${name} of ${item.shade}`} colorName={name} color={item.color} className={cn('size-10 sm:w-full lg:size-8', swatchClassName)} />
      <div className="absolute lg:left-1/2 lg:-translate-x-1/2 group-hover:block group-focus:block hidden group-pressed:block lg:top-4 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">{copied ? <IconCheck /> : <IconDuplicate />}</div>
