...
<Note intent='primary'>
  If you got other starter kits, like Remix or Vite, just drop a [PR](https://github.com/justdlabs/next.js/issues/new) and I'll slap it on the list.
</Note>
### Next.js
The easiest way to get started with Just D is to use the [Next.js Starter Kit](https://github.com/justdlabs/next.js). It's a Next.js project with justd all set up, so you can roll right away. Of just run this command:
<Code withImportCopy={false} lang='bash' code='npx create-next-app@latest -e https://github.com/justdlabs/next.js project-name'/>
...
After that, you gotta run `npm i && npm run build` in the project folder. Then, run `php artisan serve` to start the server. You're good to go.
