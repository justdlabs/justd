...
<Note intent='primary'>
  If you got other starter kits, like Remix or Vite, just drop a [PR](https://github.com/irsyadadl/justd.co/issues/new) and I'll slap it on the list.
</Note>
### Next.js
Tl;dr, just run this command:
```bash
npx create-next-app@latest -e https://github.com/justdlabs/justd-next
```
Wanna dive in? Grab the [starter kit](https://justd-nsk.vercel.app/). It's a Next.js project with justd all set up, so you can roll right away. Wanna help develop it? Don’t hold back drop a [pull request](https://github.com/justdlabs/justd-next) in the repo.
