...
## Installation
<InstallCommand isAdd items={['grid-list']}/>
...
Note that I didn't set an `id` for the grid list item since the items already have default IDs. Just assume there's no `id`, and you'll need to attach the `id` props to the grid list item like this:
```
