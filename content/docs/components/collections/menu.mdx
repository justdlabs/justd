---
title: "Menu"
description: It's the boss of options, serving up choices like a pro. Click and it’ll show you the spread pick what you like!
order: 1
published: true
references: ["https://react-spectrum.adobe.com/react-aria/Menu.html#props", "https://github.com/jamiebuilds/tailwindcss-animate"]
---

## Basic
A menu shows a bunch of actions or options you can pick from.
<How toUse="collections/menu-basic-demo" />

## Installation
<InstallCommand isAdd items={['menu']}/>

## Composed Components
<Composed components={['dropdown', 'keyboard']}/>

## Manual Installation

<InstallCommand isInstall isManual items={['react-aria-components', '@irsyadadl/paranoid']} />

<SourceCode toShow={['menu']} />

## Sub Menu
The menu can be nested to show submenus.
<How toUse="collections/menu-sub-demo" />

## Danger
You can make a menu item dangerous.
<How toUse="collections/menu-danger-demo" />

## With Icon
You can add icons to the menu items like this.
<How toUse="collections/menu-with-icon-demo" />

## Dynamic Menu
You can dynamically generate menu items.
<How toUse="collections/menu-collection-demo" />

## Multiple Menu
A menu shows a bunch of actions or options you can pick from.
<How toUse="collections/multiple-menu-demo" />

## Single Menu
A menu shows a bunch of actions or options you can pick from.
<How toUse="collections/single-menu-demo" />

## Section Menu
A menu shows a bunch of items that are grouped together.
<How toUse="collections/menu-section-demo" />

## Arrow
You add an arrow to the menu by passing `showArrow` prop to `MenuContent`.
<How toUse="collections/menu-with-arrow-demo" />

## Disabled
You can disable a menu item.
<How toUse="collections/menu-disabled-demo" />
You can also disable it by `MenuContent` by passing `disabledKeys` prop.
```tsx
<MenuContent disabledKeys={['gsu']} />
```

## Open Modal
You can open a modal when you click on a menu item. See the [modal demo](/docs/components/overlays/modal#triggered-by-menu) for more details.
