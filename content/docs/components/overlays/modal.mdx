...
## Blur
On the flip side, if your modal's packin' a `TextField` or `TextArea`, you gotta blur that overlay on iOS by default. Wanna skip the blur? Just flip the `isBlurred` prop to `false`. For all the other devices, it's clear unless you toggle `isBlurred` to `true` to get that blur going.
