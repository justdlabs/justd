...
## Installation
<InstallCommand isAdd items={['grid']}/>
...
## Link
Sometimes you need a link, like for example say you have a grid that shows a list of articles. Of course you can use link inside the group item, but also, you can use a link directly from the grid item it self, like this:
```tsx
<GridItem href='https://github.com/' />
```

## Action
You can also add actions to the grid item. The real example right now you can see is in the [Colors](/colors) page, where you can copy the color to your clipboard through the action button.
```tsx
<GridItem onAction={() => handleCopy(parseColor(item.color as string)?.toString(selectedFormat))} />
```
