---
title: "Tabs"
description: Switch between views like flipping through file folders at a desk, organizing content without clutter.
order: 3
published: true
references: ["https://react-spectrum.adobe.com/react-aria/Tabs.html#props"]
---


## Basic
Tabs divide content into sections, allowing users to easily switch between them.
<How isCenter={false} toUse="navigation/tabs-demo" />

## Installation
<InstallCommand isAdd items={['tabs']} />

## Manual Installation
<InstallCommand isInstall isManual items={['react-aria-components']} />

<SourceCode toShow='tabs' />

## With Icons
If you want to include icons in your tabs, it's simple to do. This tab setup handles icons smoothly, maintaining a clean design.
<How isCenter={false} toUse="navigation/tabs-icons-demo" />

## Orientation
You can set the orientation of the tabs to either `horizontal` or `vertical`.
<How isCenter={false} toUse="navigation/tabs-orientation-demo" />

## Collections
Tabs can also be used within collections.
<How isCenter={false} toUse="navigation/tabs-collections-demo" />

## Links
If you want to use tabs as navigational links, you can add the `href` prop to the `Tab` component. This will automatically change the cursor to a pointer.
<How isCenter={false} toUse="navigation/tabs-link-demo" />

## Responsive
When tabs get crowded, they can become difficult to manage. To keep them organized and responsive, use the following approach:

<Composed text="Mix it up with a menu, select, or combo box, but this example focuses on mastering the select component." components={['select']}/>

<How isCenter={false} toUse="navigation/tabs-responsive-demo" />

## Disabled
You can disable all tabs by setting the `isDisabled` prop to `true`.
<How isCenter={false} toUse="navigation/tabs-disabled-demo" />

You can also disable individual tabs by setting the `isDisabled` prop for each one, like this:
```tsx
<Tab isDisabled id="c">Contact</Tab>
<Tab isDisabled id="a">About Us</Tab>
```

## Framer motion
<InstallCommand isInstall isManual text='To animate the current indicator, use the motion and LayoutGroup components. First, install the package:' items={['framer-motion']} />

Next, import it in your `Tabs` component.
```tsx {1} {4} {6} {13}
import { LayoutGroup, motion } from "framer-motion"

const List = <T extends object>(props: TabListProps<T>) => {
  const id = React.useId()
  return (
    <LayoutGroup id={id}>
      <TabList
        {...props}
        className={cr(props.className, (className, renderProps) =>
          tabListStyles({ ...renderProps, className })
        )}
      />
    </LayoutGroup>
  )
}
```

And then modify the `Tab` component to use the `motion` component.
```tsx {8}
const Tab = ({ children, ...props }: TabProps) => {
  return (
    <TabPrimitive...>
      {({ isSelected }) => (
        <>
          {children}
          {isSelected && (
            <motion.span
              className={cn('...')}
            />
          )}
        </>
      )}
    </TabPrimitive>
  )
}
```
