...
## Introduction
React Aria components such as [Link](https://justd.co/d/link), [Menu](https://justd.co/d/menu), [Tabs](https://justd.co/d/tabs), and [Table](https://justd.co/d/table) transform elements into clickable links that navigate when clicked. These components utilize the `href` prop to render as an `<a>` tag, supporting attributes like `target` and `download`.
