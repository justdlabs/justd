...
? Select components to add: (Press <space> to select, <a> to toggle all, <i> to invert selection, and <enter> to
  ...
  If I were in your position, I would save this in a file named `classes.ts` within the `utils` folder. Then, whenever
  you need to use it, you can import it like this:
  ```tsx
