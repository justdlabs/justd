...
<EditorText
  ...
  ### Classes / CN
    All components that use the `cn` function will import it from the `utils/classes.ts` file. This helper file provides a `cn` function compatible with server components.
