  ...
  return (
    <div className={cn('dfakdpxe2941 not-prose group relative max-h-96 overflow-y-auto rounded-lg font-mono text-sm', className)}>
      <div className={cn('absolute dxcode z-20 bottom-auto right-3 top-3 flex gap-1.5')}>
        {withImportCopy && <CopyButton ariaLabel="Copy imports statement" initialIcon={<IconCircleInfo />} isCopied={copied === 'imports'} onPress={copyImportsToClipboard} />}
...
function CodeContainer({ children, isOpened }: { children: React.ReactNode; isOpened: boolean }) {
  return (
    <CollapsibleContent forceMount className={cn('overflow-hidden', !isOpened && 'h-32')}>
      <div className={cn('[&_pre]:my-0 [&_pre]:!border-0 [&_pre]:max-h-[32rem] [&_pre]:pb-[100px]', !isOpened ? '[&_pre]:overflow-hidden' : '[&_pre]:overflow-auto]')}>{children}</div>
    </CollapsibleContent>
  )
}
...
function CodeExpandButton({ isOpened }: { isOpened: boolean }) {
  return (
    <div className={cn('absolute flex items-center justify-center bg-gradient-to-b from-[#0e0e10]/50 to-black', isOpened ? 'inset-x-0 bottom-0 h-16' : 'inset-0')}>
...
function CodeCollapsible({ children, isOpened, onOpenChange, lang = 'tsx', withImportCopy = true, code, ...props }: React.PropsWithChildren<CodeCollapsibleProps>) {
  return (
