...
const card = tv({
  slots: {
    root: ['xrkr rounded-lg xkd2 [&:has(table)_.ccvgs8x]:border-t [&:has(table)_.x32]:bg-tertiary [&:has(table)]:overflow-hidden border bg-background text-fg shadow-sm [&:has(.larhy3):not(:has(.yahnba))>.ccvgs8x]:pt-6 [&:has(.larhy3)]:overflow-hidden [&_table]:overflow-hidden'],
...
const CardHeader = ({ className, ...props }: React.ComponentProps<typeof Header>) => <Header className={header({ className })} {...props} />
