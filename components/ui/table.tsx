...
const TableBody = <T extends object>(props: TableBodyProps<T>) => <TableBodyPrimitive {...props} className={cn('[&_.tr:last-child]:border-0')} />
  ...
  <Column {...props} className={column({ className })}>
    {({ allowsSorting, sortDirection }) => (
      <div className="flex [&>[data-slot=icon]]:shrink-0 items-center gap-2">
        <>
          {children}
          {allowsSorting && (sortDirection === undefined ? <div className="w-6" /> : sortDirection === 'ascending' ? <IconChevronUp /> : <IconChevronDown />)}
        </>
      </div>
    )}
  </Column>
...
const TableHeader = <T extends object>({ children, className, columns, ...props }: TableHeaderProps<T> & { className?: string }) => {
  const { selectionBehavior, selectionMode, allowsDragging } = useTableOptions()
  return (
    <TableHeaderPrimitive {...props} className={header({ className })}>
      {allowsDragging && <Column />}
      {selectionBehavior === 'toggle' && <Column className="pl-4">{selectionMode === 'multiple' && <Checkbox slot="selection" />}</Column>}
      <Collection items={columns}>{children}</Collection>
    </TableHeaderPrimitive>
  )
}

const TableRow = <T extends object>({ children, className, columns, id, ...props }: RowProps<T> & { className?: string }) => {
