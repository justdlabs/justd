...
import { FileTrigger as FileTriggerPrimitive, type FileTriggerProps as FileTriggerPrimitiveProps } from 'react-aria-components'
...
const FileTrigger = ({ intent = 'primary', appearance = 'outline', size = 'medium', shape = 'square', withIcon = true, ...props }: FileTriggerProps) => {
  return (
    <>
      <FileTriggerPrimitive {...props}>
        <Button isDisabled={props.isDisabled} intent={intent} size={size} shape={shape} appearance={appearance}>
          {withIcon && <>{props.defaultCamera ? <IconCamera /> : props.acceptDirectory ? <IconFolder /> : <IconPaperclip className="rotate-45" />}</>}
          {props.children ? (
            props.children
          ) : (
            <>
