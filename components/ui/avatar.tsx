...
const avatarStyles = tv({
  base: ['inline-grid relative shrink-0 bg-secondary align-middle [--avatar-radius:20%] [--ring-opacity:20%] *:col-start-1 *:row-start-1', 'loo2ppvkxrcah38e outline outline-1 -outline-offset-1 outline-black/[--ring-opacity] dark:outline-white/[--ring-opacity]'],
  variants: {
    size: {
      small: 'size-6',
      medium: 'size-8',
...
const Avatar = ({ status, src = null, initials, alt = '', children, className, shape, size, role = 'avatar', ...props }: AvatarProps) => {
  const badgeId = React.useId()
  const ariaLabelledby = [badgeId, children ? badgeId : ''].join(' ')
  return (
    <span aria-labelledby={ariaLabelledby} role={role} data-slot="avatar" {...props} className={avatarStyles({ shape, size, className })}>
      {initials && (
