...
export function DocComposed({ components, text }: { components: string[]; text?: string | React.ReactNode }) {
  ...
  return (
    <div className="not-prose">
      {!text ? (
        <>
          <p className="mb-6">When you plug this component from the CLI, it autoloads all the composed components. No need to toss 'em in one at a time.</p>
          <p className="mb-6">
            The <strong className="font-medium lowercase">{name}</strong>'s decked out with several components to make it bangin'.
          </p>
        </>
      ) : (
        ...
        <GridCollection items={filteredComponents}>
          {(item) => (
            <GridItem className="relative" id={item.slug}>
              <Link aria-label={`Open ${item.title}`} rel="noopener noreferrer" href={`/${item.slug}`} className="absolute inset-0 rounded-lg size-full" />
